...
public class DashboardController {
    ...
    public String dashboard(Model model, HttpSession session) {
        ...
        // Carrega dados atualizados do banco (evita dados stale na sessão)
        Usuario usuarioAtualizado = usuarioDAO.findByEmail(u.getEmail());
        model.addAttribute("usuario", usuarioAtualizado);

        // Lista uploads do usuário (método conforme DAO que geramos: findByUsuarioId)
        model.addAttribute("uploads", materialDAO.findByUsuarioId(usuarioAtualizado.getId()));

        // Conquistas do usuário (método conforme DAO que geramos: findByUsuario)
        model.addAttribute("conquistas", usuarioConquistaDAO.findByUsuario(usuarioAtualizado.getId()));

        // Notificações não lidas (método conforme DAO que geramos: findByUsuario)
