...
public class ConquistaDAO {
    ...
    public Integer create(Conquista c) {
        ...
        """;
        jdbc.update(sql,
            c.getTitulo(), c.getDescricao(),
            c.getPontosRecompensa(), c.getNivelRequerido(), c.getImagemUrl()
        );
    ...
    public boolean update(Conquista c) {
        String sql = """
            UPDATE conquistas
            SET titulo=?, descricao=?, pontos_recompensa=?, nivel_requerido=?, imagem_url=?
            WHERE id=?
        """;
        return jdbc.update(sql,
            c.getTitulo(), c.getDescricao(),
            c.getPontosRecompensa(), c.getNivelRequerido(),
            c.getImagemUrl(), c.getId()
        ) > 0;
    }

        public boolean delete(int id) {
        jdbc.update("DELETE FROM usuarios_conquistas WHERE conquista_id = ?", id);
        return jdbc.update("DELETE FROM conquistas WHERE id = ?", id) > 0;
    }

    public Conquista findById(int id) {
        List<Conquista> list = jdbc.query(
            "SELECT * FROM conquistas WHERE id=?", mapper, id
        );
    ...
    public List<Conquista> findAll() {
        return jdbc.query(
            "SELECT * FROM conquistas ORDER BY pontos_recompensa DESC, titulo ASC",
            mapper
        );
    }
}